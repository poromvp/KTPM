...
export function validateUsername(username) {
  if (!username) return "Không được để trống username";
  if (username.length < 3) return "Username phải có ít nhất 3 ký tự";
  if (username.length > 50) return "Username không được vượt quá 50 ký tự";
  const regex = /^[a-zA-Z0-9_]+$/;
  if (!regex.test(username)) return "Username không được chứa ký tự đặc biệt";
...
export function validatePassword(password) {
  ...
  if (password.length > 20) return "Mật khẩu không được vượt quá 20 ký tự";
